...
cases <- data.table::setDT(covidregionaldata::get_regional_data(country = "Russia"))

cases <- clean_regional_data(cases)

# Check to see if the data has been updated  ------------------------------

check_for_update(cases, last_run = here::here("last-update", "russia.rds"),
                 data = "Russia")
...
regional_epinow_with_settings(reported_cases = cases,
                ...
                no_cores = no_cores,
                target_dir = "subnational/russia/cases/national",
                summary_dir = "subnational/russia/cases/summary")
