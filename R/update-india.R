...
cases <- data.table::setDT(covidregionaldata::get_regional_data(country = "India"))

cases <- clean_regional_data(cases[, region := state])

# Check to see if the data has been updated  ------------------------------

check_for_update(cases, last_run = here::here("last-update", "india.rds"))
...
regional_epinow_with_settings(reported_cases = cases,
                ...
                no_cores = no_cores,
                target_dir = "subnational/india/cases/national",
                summary_dir = "subnational/india/cases/summary")
