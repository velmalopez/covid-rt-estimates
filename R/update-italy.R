...
cases <- data.table::setDT(covidregionaldata::get_regional_data(country = "Italy"))

cases <- clean_regional_data(cases)

# Check to see if the data has been updated  ------------------------------

check_for_update(cases, last_run = here::here("last-update", "italy.rds"),
                 data = "Italy")
...
regional_epinow_with_settings(reported_cases = cases,
                ...
                no_cores = no_cores,
                target_dir = "subnational/italy/cases/national",
                summary_dir = "subnational/italy/cases/summary")
